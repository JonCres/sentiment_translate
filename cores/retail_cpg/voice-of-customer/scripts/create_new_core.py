...
def create_ai_core(name: str, industry: str, output_dir: str = "."):
    ...
    if old_pkg_dir.exists():
        ...
        if pyproject_path.exists():
            ...
            content = content.replace('package_name = "aicore_template"', f'package_name = "{safe_name}"')
            content = content.replace('aicore_name = "AI Core Template"', f'aicore_name = "{name}"')
        ...
        if registry_path.exists():
            ...
            content = content.replace("from aicore_template.pipelines", f"from {safe_name}.pipelines")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a new AI Core from template"
    )
    parser.add_argument(
        "--name", 
        required=True, 
        help="Name of the new AI Core (e.g., fraud-detection)"
    )
    parser.add_argument(
        "--industry", 
        required=True,
        choices=["retail", "healthcare", "finance"],
