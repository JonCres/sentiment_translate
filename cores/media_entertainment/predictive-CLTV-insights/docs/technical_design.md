...
│  │  - User Filters    - AI Interpretations (SLM/Groq)            │  │
│  └──────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
...
│  │  │  - Ingestion    │  │  - BG/NBD    │  │  - Reporting    │ │  │
│  │  │  - Validation   │  │  - GammaGamma│  │  - SLM Insights │ │  │
│  │  │  - Feast Store  │  │  - XGBoost   │  │  - Plotting     │ │  │
│  │  └─────────────────┘  └──────────────┘  └─────────────────┘ │  │
│  └──────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
                                    ▼
...
| **Probabilistic ML** | Lifetimes, PyMC | Frequency/Monetary modeling (BG/NBD, Gamma-Gamma) |
| **Survival Analysis** | Lifelines | Contractual churn modeling (Weibull AFT, sBG) |
| **ML Frameworks** | XGBoost, scikit-learn | Behavioral refinement and residual prediction |
| **Explainability** | Transformers, Groq | SLM/LLM for natural language interpretation |
...
**Context:** `Media & Entertainment` requires handling both contractual (SVOD) and non-contractual (TVOD/Gaming) user behaviors alongside behavioral nuances like "binge-watching".

**Decision:** Use a multi-stage hybrid modeling approach:
1.  **Probabilistic Core:** BG/NBD and Gamma-Gamma for robust LTV baselines.
2.  **Survival Framework:** Weibull AFT for time-to-event analysis in contractual settings.
3.  **Behavioral Refinement:** XGBoost to adjust churn probabilities based on high-dimensional engagement signals.

**Rationale:**
- **BG/NBD & Gamma-Gamma:** Proven statistical methods for calculating future transaction counts and monetary value in noisy data.
- **Weibull AFT:** Allows modeling the impact of covariates (e.g., "acquisition channel") on the survival timeline.
- **XGBoost:** Captures non-linear behavioral patterns (e.g., sudden drop in usage) that probabilistic models miss.

**Consequences:**
- ✅ High accuracy across different business models (SVOD/TVOD).
- ✅ Explains *why* churn happens (via Survival covariates) and *when* (via XGBoost refinement).
- ⚠️ Increased complexity in pipeline management (multiple models).
- ⚠️ Requires specialized data preparation (RFM vectors vs. Feature Store vectors).

**Model Stack:**
```
- Non-Contractual:
  - Frequency: BG/NBD (Beta-Geometric / Negative Binomial Distribution)
  - Monetary: Gamma-Gamma Model
- Contractual:
  - Baseline: sBG (Shifted Beta Geometric)
  - Covariates: Weibull AFT (Accelerated Failure Time)
- Refinement:
  - Behavioral Churn: XGBoost Classifier (DAAL accelerated)
- Validation:
  - Bootstrapped Confidence Intervals (Parametric)
  - Kaplan-Meier Cohort Analysis
...
graph TD
    ...
    subgraph "Phase 3: Visualization Workflow"
        ...
        Node10 --> Node11(SLM Interpretations)
        Node11 --> |Output| VIZ[(Visualization Assets)]:::store
    end
...
1. **Phase 1 - Data Workflow:**
   ...
   - Engineers features and materializes to Feast

2. **Phase 2 - AI Workflow:**
   - Loads features from Feast feature store
   - Splits data for training/validation/testing
   - Trains models with hyperparameter tracking
   - Evaluates performance and registers to MLflow

3. **Phase 3 - Visualization Workflow:**
   - Generates feature distribution plots
   - Creates correlation heatmaps
   - Produces model evaluation visualizations (Frequency/Recency Matrix)
   - **AI Interpretations:** Generates natural language insights using SLMs (Small Language Models) or Groq API.
   - Exports assets for Streamlit dashboard

4. **Phase 4 - MLOps Workflow:**
   - Monitors for feature drift
...
│ │ 1. Aggregate Metrics                    │ │
│ │ 2. Generate Reports                     │ │
│ │ 3. AI Interpretations (SLM/Groq)        │ │
│ │ 4. Export to Dashboard Data Store       │ │
│ └─────────────────────────────────────────┘ │
└──────┬──────────────────────────────────────┘
       │
       ▼
...
- **Hardware Acceleration:**
    - **XGBoost:** `daal4py` (Intel CPU), `cuda` (NVIDIA), `sycl` (Intel XPU).
    - **SLM Inference:** `mps` (Apple Silicon), `xpu` (Intel Arc), `cuda` (NVIDIA).
...
- Local SLM Inference (Transformers)
