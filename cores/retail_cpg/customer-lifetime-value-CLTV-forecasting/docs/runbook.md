# Operational Runbook: Customer Lifetime Value CLTV Forecasting
...
### 1.1 Purpose
**Customer Lifetime Value CLTV Forecasting** is a production AI system that **forecasts customer value using probabilistic BTYD models (BG/NBD, Gamma-Gamma)**. It processes **10M+ transactions/year** and serves predictions to **Marketing teams, CRM managers, Finance**.
...
| **Production** | Live system serving business users | Business users, API clients | Weekly |
...
**Root Cause:**
1. Attempting to pass non-serializable objects (KedroContext, DataCatalog) between tasks.
2. **Lifetimes/BTYD models** containing lambda functions being saved with standard `pickle` instead of `cloudpickle`.

**Resolution:**
1. **For Task Passing:** Review `src/prefect_orchestration/flows.py`. Ensure tasks only pass configuration strings.
2. **For Model Saving:** Ensure `catalog.yml` uses `aicore.datasets.cloudpickle_dataset.CloudPickleDataset` for all BTYD models (BG/NBD, Gamma-Gamma).
...
**Frequency:** **Monthly** (aligned with cohort reporting cycles)
