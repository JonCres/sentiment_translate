            ...
            if skeleton.schema[col] == pl.String:
                if date_format:
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Could not auto-parse date column '{col}'. Please specify a 'date_format' in parameters.yml for subscriptions."
                        ) from e
...
def map_to_engagement_skeleton(
    ...
    for skeleton_col in final_cols:
        ...
        if skeleton_col in data.columns:
            ...
            if default_val is not None:
                ...
                logger.error(
                    f"Mandatory column '{skeleton_col}' not found in raw data, not mapped, and no default provided."
                )
                raise ValueError(
                    f"Mandatory column '{skeleton_col}' not found and cannot be derived or defaulted. Check parameters.yml mapping and defaults. Raw data columns available: {data.columns}"
                )
            else:
                # For other columns, if no default is provided, it might be an issue
                logger.warning(
                    f"Optional column '{skeleton_col}' not found, not mapped, and no default provided. It will be created with nulls."
                )
                expressions.append(
                    pl.lit(None).alias(skeleton_col)
                )  # Create with nulls if no default
    ...
    if "engagement_value" in skeleton.columns:
        ...
        skeleton = skeleton.with_columns(
            pl.col("engagement_value").cast(pl.Float64, strict=False)
        )

    return skeleton


def map_to_interaction_skeleton(
    data: pl.DataFrame, params: Dict[str, Any]
) -> pl.DataFrame:
    """Map raw data to Interaction Skeleton."""
    final_cols = [
        "customer_id",
        "event_timestamp",
        "interaction_type",
    ...
    mapping = params.get("interactions", {}).get("mapping", {})
    rename_map = {v: k for k, v in mapping.items() if v in data.columns}
    skeleton = data.rename(rename_map)

    # Cast timestamp
    if (
        "event_timestamp" in skeleton.columns
        and skeleton.schema["event_timestamp"] == pl.Utf8
    ):
        skeleton = skeleton.with_columns(
    ...
    # select final
    return skeleton.select([c for c in final_cols if c in skeleton.columns])


def map_to_qoe_skeleton(data: pl.DataFrame, params: Dict[str, Any]) -> pl.DataFrame:
    """Map raw data to QoE Skeleton."""
    final_cols = [
        "customer_id",
        "event_timestamp",
    ...
    mapping = params.get("qoe", {}).get("mapping", {})
    rename_map = {v: k for k, v in mapping.items() if v in data.columns}
    skeleton = data.rename(rename_map)
...
def map_to_advertising_skeleton(
    data: pl.DataFrame, params: Dict[str, Any]
) -> pl.DataFrame:
    """Map raw data to Advertising Skeleton."""
    final_cols = [
        "customer_id",
        "event_timestamp",
        "ad_id",
    ...
    mapping = params.get("advertising", {}).get("mapping", {})
    rename_map = {v: k for k, v in mapping.items() if v in data.columns}
    skeleton = data.rename(rename_map)
...
def map_to_tvod_skeleton(
    data: pl.DataFrame, params: Dict[str, Any]
) -> pl.DataFrame:
    """Map raw data to TVOD Skeleton."""
    final_cols = [
        "customer_id",
        "transaction_id",
        "transaction_date",
    ...
    mapping = params.get("tvod", {}).get("mapping", {})
    rename_map = {v: k for k, v in mapping.items() if v in data.columns}
    skeleton = data.rename(rename_map)

    if (
        "transaction_date" in skeleton.columns
        and skeleton.schema["transaction_date"] == pl.Utf8
    ):
        skeleton = skeleton.with_columns(
            pl.col("transaction_date").str.to_datetime(strict=False)
        )
    
    if "amount_usd" in skeleton.columns:
         skeleton = skeleton.with_columns(pl.col("amount_usd").cast(pl.Float64))
