# Operational Runbook: CLTV Predictor
...
**CLTV Predictor** is a production AI system that `forecasts long-term subscriber value across multiple revenue streams`. It processes `12M+ events/day` and serves predictions to `Finance teams, Strategic planning, Marketing`.

### 1.2 Key Components

| Component | Technology | Purpose | Location |
| :--- | :--- | :--- | :--- |
| **Data Pipelines** | Kedro 0.19+, Polars | ETL, feature engineering, model training | `src/aicore/pipelines/` |
| **Orchestration** | Prefect 2.x/3.x | Workflow scheduling and monitoring | `src/prefect_orchestration/` |
| **Feature Store** | Feast 0.38+ | Feature versioning with Delta Lake backend | `feature_repo/` |
| **Model Registry** | MLflow | Model versioning and deployment | MLflow Server |
| **Validation/Security** | Pandera, Presidio | Data validation and PII de-identification | `src/aicore/nodes.py` |
| **Dashboard** | Streamlit | Business user interface (Retention Command Center) | `app/app.py` |
| **Data Storage** | Delta Lake / S3 | High-performance atomic storage | `conf/base/catalog.yml` |
...
This project uses **Delta Lake** for atomic transactions and high-performance data storage.

| Operation | Command / Description |
| :--- | :--- |
| **Check Table History** | `polars.read_delta("path/to/table", version=N)` |
| **Vacuum Table** | `dt.vacuum(retention_hours=24)` (Removes old files) |
| **Optimize Table** | `dt.optimize.compact()` (Merge small files) |
...
#### **Symptom:** Delta Table is locked or has concurrent write error

**Diagnosis:**

```bash
# Check for .checkpoint and _delta_log/ files
ls -R data/03_primary/my_table/_delta_log/
```

**Resolution:**

1. Wait for existing write process to complete
2. If process crashed, manually check for stale lock files (rare in modern Delta implementations)
3. Ensure no two Prefect flows are writing to the same Delta table concurrently
...
```bash
# Verify that Presidio is correctly masking data in primary layer
kedro run --pipeline data_processing --to-nodes mask_pii_node
```
