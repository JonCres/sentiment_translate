...
flow_name = f"{ai_core_name}_ai_flow"

class AIPipeline(KedroPipeline):
    """AI Pipeline: Train, Evaluate, Register using Kedro"""
    pipeline_name = "data_science"

@flow(name=flow_name, log_prints=True)
def run_pipeline(config: Dict[str, Any]):
    """Entry point flow that instantiates and runs the AIPipeline"""
    pipeline = AIPipeline(config)
    pipeline.run()

if __name__ == "__main__":    
    deployment_config = config["deployments"]["ai_pipeline"]
