...
| **Feature Store** | Feast 0.38+ | Feature versioning and serving (Delta Lake backend) |
| **Model Registry** | MLflow | Model versioning, tracking, and deployment |
| **ML Frameworks** | PyTorch, XGBoost, LightGBM, pymc-marketing, scikit-survival | Survival (DeepSurv), BTYD (BG/NBD), and Ensemble modeling |
| **Data Processing** | Polars, Pandas, Delta Lake | High-performance data manipulation and atomic storage |
| **Validation & Security** | Pandera, Pydantic, Microsoft Presidio | Data validation and PII de-identification/masking |
...
**Context:** `Media & Entertainment` has unique business physics and data characteristics, requiring a transition from binary churn labels to a unified value framework.

**Decision:** Deploy a **Unified Subscriber Value Framework** that integrates **Temporal Hazard Modeling** (Survival Analysis) for SVOD retention with **Probabilistic BTYD Models** (BG/NBD + Gamma-Gamma) for TVOD/Gaming monetization.

**Rationale:**

- **Business Justification:** A subscriber's value is the integral of their survival function $S(t)$ multiplied by their multi-channel revenue stream (SVOD + AVOD + TVOD).
- **Technical Justification:** Standard regressions fail to account for "right-censoring" (active users). Survival models (DeepSurv) and Bayesian BTYD models provide mathematically rigorous foundations for LTV.
- **Domain-Specific Requirements:** Must integrate Quality of Experience (QoE) telemetry and "Watch-time Velocity" as leading indicators of both hazard $h(t)$ and upsell potential.

**Consequences:**

- ✅ **Precision:** Accurate NPV calculation across the customer lifecycle.
- ✅ **Actionability:** Enables interventions within the optimal 14-21 day "Pre-Churn" window.
- ⚠️ **Complexity:** Requires specialized handling of time-varying covariates and censoring logic.
- ⚠️ **Data Intensity:** Demands high-velocity ingestion of telemetry for QoE-based features.

**Model Stack:**

- **Survival Track:** DeepSurv (Neural Proportional Hazards), Random Survival Forests (RSF), and Cox-PH via `scikit-survival`.
- **Value Track:** BG/NBD (frequency) and Gamma-Gamma (monetary) models via `pymc-marketing`.
- **Sequential Layer:** CNN-BiLSTM with Multi-Head Self-Attention for processing viewing sequences.
- **Validation Metrics:** Concordance Index (C-index), Brier Scores, and MAPE (Mean Absolute Percentage Error).
...
The following Mermaid diagram illustrates the four-phase workflow orchestration, reflecting the **Unified Subscriber Value Framework**:
...
graph TD
    ...
    subgraph "Phase 1: Data Workflow"
        ...
        KP_DP --> Node1(Skeletons: Txns, Subs, QoE, Adv)
        Node1 --> Node2(Cleaning & Transformation)
        Node2 --> Node3(Feature Store & Survival Data)
        Node2 --> Node4(DL Tensor Sequences)
        Node3 & Node4 --> Node5(Feast Registration)
        Node5 -->|Output| FS[(Feast Feature Store)]:::store
    end

    subgraph "Phase 2: AI Workflow"
        AP[Prefect: AIPipeline]:::prefect -->|Orchestrates| KP_DS[Kedro: data_science]:::kedro
        FS --> KP_DS
        KP_DS --> Track1(Survival Track: DeepSurv, RSF)
        KP_DS --> Track2(Value Track: BTYD Models)
        KP_DS --> Track3(Sequential Track: BiLSTM-Attn)
        Track1 --> XAI(Explainability: SHAP/LIME)
        Track1 & Track2 & Track3 & XAI --> Node7(Ensemble predict_cltv)
        Node7 -->|Output| MR[(MLflow Model Registry)]:::store
    end
    ...
    subgraph "Phase 3: Visualization Workflow"
        ...
        FS --> KP_VIZ
        MR --> KP_VIZ
        KP_VIZ --> Node8(Hazard/Survival Curves)
        Node8 --> Node9(XAI Heatmaps)
        Node9 --> Node10(CLTV Risk Funnels)
        Node10 --> |Output| VIZ[(Visualization Assets)]:::store
    end

    subgraph "Phase 4: MLOps Workflow"
        MP[Prefect: MonitoringPipeline]:::prefect --> |Executes| Task1(Check Semantic Drift)
...
1. **Phase 1 - Data Workflow:**
   - Ingests raw data into domain-specific skeletons (Transactions, Subscriptions, QoE, Advertising).
   - Cleans and transforms raw logs into a unified interaction backbone.
   - Engineers temporal features and prepares longitudinal survival datasets.
   - Generates tensor sequences for deep learning and materializes features to **Feast**.

2. **Phase 2 - AI Workflow:**
   - **Survival Track:** Trains time-to-event models (DeepSurv, RSF) to calculate hazard rates.
   - **Value Track:** Trains BG/NBD and Gamma-Gamma models for monetization forecasting.
   - **Sequential Track:** Processes engagement velocity using CNN-BiLSTM with Attention.
   - **Explainability:** Generates SHAP and LIME global/local importance scores.
   - **Ensemble:** Integrates all tracks into a final CLTV prediction registered to **MLflow**.

3. **Phase 3 - Visualization Workflow:**
   - Generates individual survival probability curves and hazard ratio plots.
   - Creates explainability heatmaps (SHAP summary plots) for business interpretability.
   - Produces CLTV Risk Funnels and Acquisition Cohort analysis scripts.
   - Exports high-fidelity assets for the **Streamlit** dashboard.

4. **Phase 4 - MLOps Workflow:**
   - Monitors for feature and semantic drift in interaction patterns.
   - Validates model calibration (predicted vs. actual survival probabilities).
   - Triggers re-training flows if C-index drops below defined thresholds.
   - Logs performance metadata to MLflow for long-term health tracking.
...
- **Data Volume:** 100M+ event streams/day
- **Inference Latency:** <200ms per subscriber prediction
- **Concurrent Users:** 50+ concurrent dashboard sessions
