# Operational Runbook: Predictive CLTV Insights

**For Support Teams & On-Call Engineers**  
**Version:** 1.0.1 | **Last Updated:** 2026-01-22
...
### 1.1 Purpose
**Predictive CLTV Insights** is a production AI system that `predicts subscriber lifetime value by integrating temporal hazard modeling with multi-stream CLTV forecasting`. It processes `10M+ events/day` and serves predictions to `Marketing teams, Product managers, Finance analysts`.
...
| **Explainability** | Transformers/Groq | Natural language insight generation | `src/aicore/pipelines/visualization/` |
...
└────────┬────────┘     └──────────────┘
         ...
         ▼
┌─────────────────┐     ┌──────────────┐     ┌──────────────┐
│ Prefect Server  │────▶│ MLflow Server│────▶│ Groq API /   │ (Optional)
└────────┬────────┘     └──────────────┘     │ Local SLM    │
         │                                   └──────────────┘
         ▼
┌─────────────────┐
...
| **SLM Failure** | Interpretation generation fails | 🟡 Warning | Check API keys / GPU Memory |
...
### 5.4 SLM/AI Insight Issues

#### **Symptom:** "AI Interpretations skipped" or missing in dashboard

**Diagnosis:**
```bash
# Check logs for Groq/SLM errors
grep "Groq" logs/info.log
grep "SLM" logs/info.log
```

**Resolution:**
1. **Check API Keys:**
   - Ensure `GROQ_API_KEY` is set in `conf/local/credentials.yml` or environment.
2. **Check GPU Availability (Local SLM):**
   - If running local Transformers, ensure GPU memory is sufficient (>8GB for Phi-4).
   - Verify `torch.cuda.is_available()` is True.
3. **Fallback:**
   - Switch to a smaller model in `parameters.yml` (e.g., `microsoft/Phi-3-mini`).

---

### 5.5 Feature Store Issues
