# Operational Runbook: Churn Forecasting
...
**Churn Forecasting** is a production AI system that `identifies subscribers likely to cancel 30-90 days before the event occurs`. It processes `5M+ events/day` and serves predictions to `Retention teams, Customer success, Marketing`.
...
| **Inference & DL** | PyTorch, XGBoost | Hybrid DL and Ensemble models | `src/aicore/pipelines/data_science` |
...
| **Production** | Live system serving business users | Business users, API clients | Daily (4 AM UTC) |
...
| **Model Drift (PSI)** | <0.1 | 0.1-0.2 | >0.25 |
...
prefect deployment run "Churn Forecasting/run_all_pipelines"
...
# Or use CLI
mlflow models list --registered-model-name "churn_prediction_model"
```
...
```bash
# Override parameters
uv run kedro run --params "modeling.model_type=ensemble,data_processing.observation_period_end=2026-01-17"
```
...
```bash
# Promotion via MLflow UI:
# 1. Navigate to Models → churn_prediction_model
# 2. Select version → Transition to → Production

# Or via CLI
uv run mlflow models transition \
...
# Get model details
uv run mlflow models describe --name "churn_prediction_model"
...
with mlflow.start_run(experiment_id=get_or_create_experiment("churn_prediction")):
...
cat conf/base/catalog.yml | grep churn_predictions

# Verify file exists
ls -lh data/01_raw/churn_data.csv
```
...
4. **Promote to production if validated:**
   ...
   ```bash
   uv run mlflow models transition --name "churn_prediction_model" --version [new_version] --stage Production
   ```
...
uv run mlflow models list --registered-model-name "churn_prediction_model"
...
**Frequency:** Weekly (to capture evolving viewing patterns)
...
```bash
# Kedro
uv run kedro run                                    # Run all pipelines
uv run kedro run --pipeline [name]                  # Run specific pipeline
uv run kedro catalog list                           # List all datasets
uv run kedro viz                                    # Visualize pipeline

# Prefect
prefect deployment run "Churn Forecasting/run_all_pipelines" # Trigger deployment
prefect flow-run logs <id>                   # View logs
prefect agent start -q default               # Start agent

# MLflow
uv run mlflow ui --port 5000                         # Start UI
uv run mlflow models list                           # List models
uv run mlflow models transition --name [name] --version [v] --stage Production

# Feast
cd feature_repo
uv run feast feature-views list                     # List feature views
uv run feast materialize-incremental [timestamp]    # Materialize features
uv run feast serve                                  # Start feature server

# Streamlit
uv run streamlit run app/app.py                     # Start dashboard
uv run streamlit cache clear                        # Clear cache
```
