...
flow_name = f"{ai_core_name}_data_flow"

class DataPipeline(KedroPipeline):
    """Data Pipeline: Extract, Validate, Clean, Feature Engineering using Kedro"""
    pipeline_name = "data_processing"

@flow(name=flow_name, log_prints=True)
def run_pipeline(config: Dict[str, Any]):
    """Entry point flow that instantiates and runs the DataPipeline"""
    pipeline = DataPipeline(config)
    pipeline.run()

if __name__ == "__main__":
    deployment_config = config["deployments"]["data_pipeline"]
