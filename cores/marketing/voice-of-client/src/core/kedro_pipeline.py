...
class KedroPipeline(ABC):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.ai_core_name = self.config.get('project', {}).get('name', 'ai_core_project_name').lower().replace(' ', '_')
        self.logger = None
        self.kedro_handlers = []        
        ...
        if self.pipeline_name is not None:
            ...
            self.run = flow(name=f"{self.ai_core_name}_kedro_{self.pipeline_name}_pipeline", log_prints=True)(original_run)
        else:
    ...
    def setup_kedro_logging(self, prefect_logger) -> List[logging.Handler]:
        ...
        ai_core_logger.addHandler(prefect_handler)
        handlers.append((ai_core_logger, prefect_handler))
    ...
    def run(self):
        ...
        with KedroSession.create(project_path=project_path) as session:
            ...
            session.store["pipeline_name"] = self.pipeline_name # Ensure pipeline_name is in store
