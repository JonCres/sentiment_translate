...
flow_name = f"{ai_core_name}_monitoring_flow"
...
class MonitoringPipeline(KedroPipeline):
    ...
    def setup(self):
        ...
        super().setup()
        # Configure MLflow from Kedro parameters
    ...
    def run(self):
        ...
        mlops_config = self.config.get('kedro_params', {}).get('mlops', {})
        experiment_name = mlops_config.get('experiment_name', 'monitoring')

        # Start MLflow run and execute Kedro pipeline within its context
        mlflow.set_experiment(experiment_name)
        with mlflow.start_run(run_name=f"{self.ai_core_name}_monitoring"):
            # Link Prefect run ID to MLflow
            link_prefect_run_id()

            # Execute the Kedro pipeline (inherits active MLflow run)
            super().run()
