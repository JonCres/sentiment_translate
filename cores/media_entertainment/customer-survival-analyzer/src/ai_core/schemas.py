...
class InteractionSchema(pa.DataFrameModel):
    ...
    customer_id: str
    event_timestamp: pl.Datetime
    interaction_type: str  # search, watchlist, rating, share
    content_id: str
...
class CountingProcessSchema(pa.DataFrameModel):
    """Schema for Counting Process Data (Time-Varying Covariates)."""

    customer_id: str
    t_start: int = pa.Field(ge=0)
    t_stop: int = pa.Field(ge=0)
    event: int = pa.Field(isin=[0, 1])
