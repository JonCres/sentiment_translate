...
description: "Detailed technical specifications, architectural decisions, and implementation patterns for the voice-of-client system."
audience: developer
...
### 2.5 Decision: Multi-Modal Voice of the Client Strategy

**Context:** Marketing departments need to synthesize fragmented feedback from various channels to understand the emotional and practical drivers of client satisfaction.

**Decision:** Implement a multi-model ensemble including Aspect-Based Sentiment Analysis (ABSA), Multimodal Emotion Recognition (MER), and Predictive Rating Models correlated with NPS/CSAT.

**Rationale:**

- **ABSA** identifies specific pain points (e.g., pricing vs. feature set).
- **MER** captures raw emotional signals from audio and video interactions.
- **Predictive Ratings** fill gaps for clients who don't provide explicit scores.
- **NPS/CSAT Integration** validates AI insights against established business KPIs.

**Consequences:**

- ✅ Granular visibility into client sentiment drivers.
- ✅ Proactive identification of churn risk using behavioral proxies.
- ⚠️ Higher computational complexity for multimodal processing.
- ⚠️ Requires robust data alignment across disparate channels.

**Model Stack:**

- **Text Layer:** HuggingFace Transformers (RoBERTa/DistilBERT) for ABSA.
- **Emotion Layer:** Multimodal Emotion Recognition (MER) using AVT-CA fusion.
- **Topic Layer:** Latent Dirichlet Allocation (LDA) for emerging theme discovery.
- **Predictive Layer:** XGBoost for inferring satisfaction ratings and churn risk.
- **Validation:** Confusion Matrix, F1-Score, NPS/CSAT Correlation.
...
The `Voice of the Client` AI Core follows a hybrid structure that separates orchestration concerns from pipeline logic:
...
graph TD
    ...
    subgraph "Phase 1: Data Workflow"
        ...
        Node2 --> Node2_5(Language Detection)
        Node2_5 --> Node2_6(Translation)
        Node2_6 --> Node3(Clean)
...
class MyCustomDataset(AbstractDataset):
    ...
    def _load(self) -> pl.DataFrame:
        # Custom load logic
        return pl.read_delta(self._filepath)
        
    def _save(self, data: pl.DataFrame) -> None:
        # Custom save logic
        data.write_delta(self._filepath)
