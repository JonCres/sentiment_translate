...
class MultiDeviceManager:
    ...
    Uses native PyTorch XPU support (PyTorch 2.6+) for Intel Arc GPUs.
    Intel Extension for PyTorch (IPEX) has reached EOL and is no longer required.

    Supported Platforms:
    - Windows: Native XPU via PyTorch XPU backend
    - Linux: Native XPU via PyTorch XPU backend (requires Intel compute runtime)

    Note: IPEX is deprecated as of 2025. PyTorch now handles XPU natively.
    """
    ...
    def __init__(self):
        ...
        if self.device_type == DeviceType.XPU:
            # IPEX is deprecated (EOL). PyTorch 2.6+ has native XPU support.
            # Attempting import only for legacy compatibility.
            try:
                import intel_extension_for_pytorch as ipex

                self.ipex = ipex
                logger.warning(
                    "Intel Extension for PyTorch (IPEX) is deprecated. "
                    "Using native torch.xpu is recommended."
                )
            except ImportError:
                # Expected path: Native XPU support (PyTorch 2.6+)
                logger.info("Using native PyTorch XPU support (torch.xpu).")
    ...
    def optimize_model(self, model, dtype=torch.float16):
        ...
        Note: For XPU, native PyTorch 2.6+ handles optimizations automatically.
        IPEX optimization is deprecated and only used for legacy compatibility.
        """
        ...
        if self.device_type == DeviceType.XPU:
            if self.ipex:
                try:
                    ...
                    logger.info(f"✓ IPEX optimization applied (deprecated, dtype={dtype})")
                except Exception as e:
                    logger.warning(f"Failed to apply IPEX optimization: {e}")
            else:
                logger.info(f"✓ Native XPU configured (dtype={dtype})")
