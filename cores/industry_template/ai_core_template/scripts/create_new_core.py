...
def create_ai_core(name: str, industry: str, output_dir: str = "."):
    ...
    feature_store_data_dir = new_core_dir / "feature_repo/data/feature_store"
    feature_store_data_dir.mkdir(parents=True, exist_ok=True)
    (feature_store_data_dir / ".gitkeep").touch()
    print(f"✅ Created feature store data directories")
    ...
    if pyproject_path.exists():
        ...
        content = content.replace(
            'aicore_name = "AI Core Template"', f'aicore_name = "{name.replace("-", " ").title()}"'
        )
    ...
    # 7. Ensure pipeline hooks are at canonical location (src/ai_core/hooks.py)
    hooks_canonical = new_pkg_dir / "hooks.py"
    hooks_legacy = new_pkg_dir / "pipelines" / "hooks.py"
    if hooks_legacy.exists():
        if not hooks_canonical.exists():
            shutil.move(str(hooks_legacy), str(hooks_canonical))
            print("✅ Moved pipeline hooks to canonical location (src/ai_core/hooks.py)")
        else:
            hooks_legacy.unlink()
            print("✅ Removed legacy pipelines/hooks.py (canonical hooks.py already exists)")

    # Confirm pipeline execution hooks are present
    if hooks_canonical.exists():
        print("✅ Pipeline execution hooks configured")
    else:
        print("⚠️  Pipeline hooks not found at src/ai_core/hooks.py")

    # Confirm Universal Device Management is present
    device_manager = new_core_dir / "src/utils/device.py"
    if device_manager.exists():
        print("✅ Universal Device Management configured (CUDA/XPU/MPS/CPU)")
    else:
        print("⚠️  Device manager not found at src/utils/device.py")
