...
class SkeletonParams(BaseModel):
    ...
    observation_period_end: Optional[str] = Field(
        None, description="End date for observation period YYYY-MM-DD"
    )
    customer_id_col: str = Field(
        "customer_id", description="Name of the customer ID column"
    )
...
class TransactionSchema(pa.DataFrameModel):
    ...
    amount_usd: float = pa.Field(
        ge=0.0, description="Transaction amount must be non-negative"
    )
...
class RFMSchema(pa.DataFrameModel):
    ...
    inactive_duration: float = pa.Field(ge=0.0)
    churn_label: int = pa.Field(isin=[0, 1])
    T: float = pa.Field(ge=0.0, description="Tenure/Age of the customer")

    class Config:
        strict = False


class InteractionSchema(pa.DataFrameModel):
    """Schema for User Interaction and Engagement."""

    customer_id: str
    event_timestamp: pl.Datetime
    interaction_type: str  # search, watchlist, rating, share, binge
    content_id: str
    session_duration_min: float = pa.Field(ge=0.0)
    device_type: str
...
class AdvertisingSchema(pa.DataFrameModel):
    """Schema for Advertising Metrics (AVOD/FAST)."""

    customer_id: str
    event_timestamp: pl.Datetime
    ad_id: str
    ad_type: str  # pre-roll, mid-roll, post-roll
    ad_completed: bool
    ad_duration_sec: float = pa.Field(ge=0.0)
    cpm_usd: float = pa.Field(ge=0.0)

    class Config:
        strict = False


class QoESchema(pa.DataFrameModel):
    """Schema for Quality of Experience (QoE) Metrics."""

    customer_id: str
    event_timestamp: pl.Datetime
    buffering_duration_sec: float = pa.Field(ge=0.0)
    startup_time_sec: float = pa.Field(ge=0.0)
    bitrate_kbps: float = pa.Field(ge=0.0)
    error_count: int = pa.Field(ge=0)
