# Operational Runbook: Customer Survival Analyzer
...
**Customer Survival Analyzer** is a production AI system that `predicts when a customer will churn using survival analysis techniques`. It processes `8M+ events/day` and serves predictions to `Data science teams, Retention specialists`.

### 1.2 Key Components

| Component | Technology | Purpose | Location |
|-----------|-----------|---------|----------|
| **Data Pipelines** | Kedro 0.19+ | ETL, feature engineering, survival modeling | `src/ai_core/pipelines/` |
| **Orchestration** | Prefect 2.x/3.x | Workflow scheduling and monitoring | `src/prefect_orchestration/` |
| **Feature Store** | Feast + Delta Lake | High-performance feature storage & versioning | `feature_repo/` |
| **Model Registry** | MLflow | Versioning for CPH, RSF, and DeepSurv models | MLflow Server |
| **Validation** | Pandera, Presidio | Schema validation and PII de-identification | `src/ai_core/schemas.py` |
| **Dashboard** | Streamlit | Survival Intelligence Hub (Retention Center) | `app/app.py` |
| **Data Storage** | Delta Lake / S3 | Raw data, features, and survival metrics | `conf/base/catalog.yml` |
...
| **Survival C-Index** | >0.78 | 0.70-0.78 | <0.70 |
| **Brier Score (90d)** | <0.15 | 0.15-0.25 | >0.25 |
| **Data Freshness** | <24 hours | 24-48 hours | >48 hours |
| **PII Leakage Check** | 0 detections | N/A | >0 detections |
| **Model Calibration (PSI)** | <0.1 | 0.1-0.25 | >0.25 |
| **Hazard Calculation Latency**| <150ms | 150-300ms | >300ms |
...
# 1. Run data processing pipeline (includes PII masking)
kedro run --pipeline data_processing

# 2. Materialize features to Delta Lake
# This is handled automatically by Kedro nodes, but verify via:
ls -lh data/04_feature/*.delta

# 3. Apply changes to Feast
cd feature_repo
feast apply
feast materialize-incremental $(date -u +"%Y-%m-%dT%H:%M:%S")
```
...
### 7.4 PII Anonymization Audit

**Frequency:** Monthly or after schema changes

**Procedure:**

1. **Run PII vulnerability scan:**

   ```bash
   # Check if PII masking node is effective
   kedro run --pipeline data_processing --nodes mask_pii_node
   ```

2. **Verify primary data samples:**
   - Inspect `data/03_primary/` samples to ensure no email, name, or exact IP is present.
   - Use `Presidio` analyzer directly on a sample file to check for leakage.

3. **Log audit result:**
   - Document any findings and update the `Presidio` regex patterns in `src/ai_core/pipelines/data_processing/nodes.py` if needed.
