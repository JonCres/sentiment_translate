# Operational Runbook: Voice of Customer AI
...
### 1.1 Purpose
**Voice of Customer AI** is a production AI system that `extracts actionable insights from customer reviews, surveys, and feedback`. It processes `1M+ reviews/month` and serves predictions to `Product teams, Customer experience, Marketing`.

### 1.2 Key Components

| Component | Technology | Purpose | Location |
|-----------|-----------|---------|----------|
| **Data Pipelines** | Kedro 0.19+ | ETL, feature engineering, model training | `src/aicore/pipelines/` |
| **Sentiment Core** | HuggingFace | Sentiment Analysis, Customer Profiling | `src/aicore/pipelines/data_science/sentiment_analysis` |
| **ABSA Core** | DeBERTa | Aspect extraction and sentiment scoring | `src/aicore/pipelines/data_science/absa` |
| **Topic Core** | BERTopic | Unsupervised topic discovery and labeling | `src/aicore/pipelines/data_science/topic_modeling` |
| **Unified Insights** | Kedro | Merging all outputs, deriving Actions/Urgency | `src/aicore/pipelines/data_science/unified_insights` |
...
api_keys:
  groq_api_key: ${GROQ_API_KEY} # For LLM interpretation
```
...
| **Sentiment Drift** | < 10% shift | 10-20% shift | > 20% shift |
| **Urgency Rate** | < 5% High Urgency | > 10% | > 15% (Crisis) |
| **Topic Coherence** | > 0.4 | 0.3-0.4 | < 0.3 |
...
# Or use CLI
mlflow models list --registered-model-name "absa_model"
```
...
**Alert Channels:**
- **Email:** `oncall@aicore.example.com`
- **Slack:** `#ai-core-alerts`
- **PagerDuty:** `PAGERDUTY_KEY_PLACEHOLDER`
...
| **Sentiment Drift** | Max sentiment shift > 20% | 🟡 Warning | Schedule retraining or check data source |
| **High Urgency** | > 15% interactions flagged High | 🔴 Critical | Alert Product/CS teams (Potential crisis) |
...
### 4.3 Training a New Topic Model

If topics are becoming stale or generic, you can retrain the BERTopic model specifically:

1.  **Update Parameters:** Check `conf/base/parameters.yml` under `topic_modeling`. You might want to adjust `nr_topics` or `min_topic_size`.
2.  **Run Pipeline:**
    ```bash
    kedro run --pipeline topic_modeling
    ```
3.  **Verify:** Check MLflow for the new run and topic coherence metrics. The model will be saved to `data/06_models/topic_model.pkl`.
...
# 4. Start local MLflow server (use uv run for proper environment)
 הראשון uv run mlflow server --host 127.0.0.1 --port 5000
...
```bash
# Open MLflow UI
 הראשון uv run mlflow ui --port 5000
...
**Diagnosis:**
```bash
# Check system logs or Prefect logs
# Look for "Killed" or "CUDA out of memory"
```

**Cause:** NLP models (DeBERTa, BERTopic) are memory intensive.

**Resolution:**
1.  **Decrease Batch Size:** In `conf/base/parameters.yml`, reduce `batch_size` for `absa` or `sentiment_analysis` (e.g., from 32 to 8).
2.  **Enable Gradient Checkpointing:** If training, enable this option in model parameters.
3.  **Use CPU Offloading:** Ensure `low_cpu_mem_usage=True` is set in model loading (default in `absa` pipeline).
...
#### **Symptom:** Delta Lake / Polars Issues on Windows

**Symptom:** `OSError: atomic rename` or `Upload aborted` when writing to `data/04_feature/*.delta`.

**Cause:** Known issue with `deltalake` writer on Windows/WSL mounts (`/mnt/c`).

**Resolution:**
1.  Ensure you are using `aicore.datasets.polars_delta_dataset.PolarsDeltaDataset` in `catalog.yml`.
2.  This custom dataset implements a workaround (writes to `/tmp` native Linux FS first, then copies to target).
